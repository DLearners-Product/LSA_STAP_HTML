...
public class Html_values
{
    ...
    public Html_values(string[] slideName, string[] teacherInst, bool[] videoSlides, bool[] worksheetSlides, bool[] syllableSlides, bool[] grammerSlide)
    {
        myLists = new List<Html_List>();
        for (int i = 0; i < slideName.Length; i++)
        {
            myLists.Add(new Html_List(i + 1, slideName[i], teacherInst[i], videoSlides[i], worksheetSlides[i], syllableSlides[i], grammerSlide[i]));
        };
...
public class Html_List
{
    ...
    public Html_List(int slideNo, string slideName, string teacherInst, bool hasVideo, bool hasWorksheet, bool hasSyllable, bool hasGrammer)
    {
